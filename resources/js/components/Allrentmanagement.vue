<template>
    <div class="container-fluid">
        ...
        <section v-if="$gate.inventory_is_NOTnull()">
            <div class="row">
                <div class="col-sm-6">
                    <h3 style="color:#34495e"> RENT MANAGEMENT - All</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item">
                ...
                <div class="col-md-2">
                    <div class="row mt-3 mb-4">
                        ...
                        <div class="col-md-12 pb-2" v-if="$gate.rent_Add_() | $gate.rent_Add_Edit() | $gate.rent_Add_Delete() | $gate.rent_Add_Edit_Delete()">
                            <button class="btn btn-primary btn-block" data-toggle="modal" data-target="#addPayment"
                                @click="addPayment">
                                <i class="fa fa-plus" aria-hidden="true"></i> Add payment
                            </button>
                        </div>
                        <div class="col-md-12 pb-2" v-if="$gate.rent_Add_() | $gate.rent_Add_Edit() | $gate.rent_Add_Delete() | $gate.rent_Add_Edit_Delete()">
                            <button class="btn btn-success btn-block" data-toggle="modal" data-target="#addPayment"
                            ...
                            <div class="col-md-12 ">
                                ...
                                <div class="form-row">
                                    <div class="col">
                                        ...
                                        <div class="form-group clearfix">
                                            <input type="date" class="form-control" v-model="searchDueDateForm.dateFrom"
                                                :class="{ 'is-invalid': searchDueDateForm.errors.has('dateFrom') }"
                                                name="dateFrom">
                                        </div>
                                    </div>
                                    <div class="col">
                                        ...
                                        <div class="form-group clearfix">
                                            <input type="date" class="form-control" v-model="searchDueDateForm.dateTo"
                                                :class="{ 'is-invalid': searchDueDateForm.errors.has('dateTo') }"
                                                name="dateTo">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group clearfix">
                                    ...
                                    <select name="sort" id="role" class="form-control" v-model="searchDueDateForm.sort">
                                        ...
                                        <option value="brnch.name">Branch</option>
                                        <option value="b.box_number">Box #</option>
                                        <option value="r.due_date">Payment due date</option>
                                ...
                                <!-- <div class="form-group clearfix">
                                    <label class="pr-2"> Sort by</label>
                                    <select name="sortBy" id="role" class="form-control"
                                        v-model="searchDueDateForm.sortBy">
                                        <option value>Select</option>
                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <div class="card-body">
                            <table class="table">
                                ...
                                <tbody>
                                    <tr>
                                    <tr v-for="(rent, index) in rents.data" :key="index"
                                        v-bind:class="{ '' : rent.status == 'Due date' , '' : rent.status == 'Over due' }">
                                        <td>{{rent.fullname}}</td>
                                        <td>{{rent.branch}}</td>
                                        <td>{{rent.box_number}}</td>
                                        <td>{{rent.due_date | myDate}}</td>
        ...
        <!-- GENERATE BILL -->
        <!-- Modal -->
        <div class="modal fade" id="generateBill" tabindex="-1" role="dialog" aria-labelledby="generateBill"
            aria-hidden="true">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="month" class="form-control" v-model="genereBillForm.date"
                                        :class="{ 'is-invalid': genereBillForm.errors.has('date') }" name="date">
                                </div>
                            </div>
        ...
        <section>
            <div class="modal fade" id="addPayment" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel" v-show="paymentAdd" v-if="!editmode">Add
                                payment</h5>
                            <h5 class="modal-title" id="exampleModalLabel" v-show="!paymentAdd" v-if="!editmode">Advance
                                payment</h5>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <v-select :options="renters" :reduce="item => item.id" label="item_data"
                                                placeholder="Select renter" v-model="form.box_id" name="box_id"
                                                :on-search="loadRenters" :value="form.box_id"
                                                v-on:input="clickSearchResult"
                                                :class="{ 'is-invalid': form.errors.has('box_id') }"></v-select>
                                    ...
                                    <div class="col-md-12" v-if="!paymentAdd">
                                        <div class="form-group">
                                            <label>Month covered</label>
                                            <input type="date" class="form-control" v-model="form.monthcovered"
                                                :class="{ 'is-invalid': form.errors.has('monthcovered') }"
                                                name="monthcovered">
                                        </div>
                                    </div>
                                    <div class="col-md-12" v-if="paymentAdd">
                                        <div class="form-group">
                                            <label>Month covered</label>
                                            <select name="monthcovered" v-model="form.monthcovered" id="role"
                                                class="form-control"
                                                :class="{ 'is-invalid': form.errors.has('monthcovered') }">
                                                <option value>Select</option>
                                                <option v-for="item in monthlist" :value="item.due_date" :key="item.id">
                                                    {{ item.due_date | billingDate}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
    ...
    export default {
        ...
        methods: {
            ...
            editItem(item) {
                ...
                this.form.fill(item);
                axios.post("api/monthcoveredlist?id=" + this.form.box_id).then(({
                    data
                }) => (this.monthlist = data));
            },
            editPayment() {
                this.form
                    ...
                    .then((data) => {
                        ...
                        Swal.fire("Updated!", data.data, "success");
                        Fire.$emit("AfterCreate");
                    })
                    .catch(() => {
                        Swal.fire({
            ...
            clickSearchResult() {
                ...
                this.form.post("api/monthcoveredlist?id=" + box_id['boxid']).then(({
                    data
                }) => (this.monthlist = data));
            ...
            loadPaymentAmount(id) {
                axios.post("api/paymentrentamount?id=" + id).then(({
                    data
                }) => {
            ...
            loadRenters: function () {
                axios.get("api/boxlistlocation").then(({
                    data
                }) => (
                    this.renters = data,
                    this.renters.map(item => {
                        return item.item_data = item.fullname + '-' + item.branch + '-' + item
                            .box_number;
                    })
                ));
            },
