<template>
    <div class="container">
       ...
       <div class="row justify-content-center" >
             <form @submit.prevent="searchReturn" class="form-inline float-sm-right" style="width:100%">
        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="card mt-2" v-for="item in returnProd.data" :key="item.id ">
                    <div class="card-body" style="padding:15px;border-left:4px solid #2c3e50">
                        <div class="row">
                          <div class="col-md-12">
                            <label>Date:</label> {{item.date | myDate}}
                          </div>
                           <div class="col-md-12">
                           ...
                           <div class="col-md-12">
                            <label>Total return:</label> {{item.quantity}}
                          </div>
                        </div>
                    </div>
                </div>
      ...
      methods: {
        searchReturn: function(page = 1){
          ...
          this.branchid = this.$gate.Userprofile();
            this.searchForm.post("api/renterreturns?userid=" + this.userid.renter_id + "&branchid=" + this.branchid.branch_id +  "&page=" + page ).then(({ data }) => {
              this.returnProd = data;
            });
        }
      },
      created() {
