    ...
    <div class="container-fluid">
     <section  v-if="$gate.rent_null()">
        ...
        <section v-if="$gate.inventory_is_NOTnull()">
            <div class="row">
                 <div class="col-sm-6">
                      <h3 style="color:#34495e"> RENT MANAGEMENT - DUES</h3>
                    </div>
                <div class="col-md-6">
                      <ol class="breadcrumb float-sm-right">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                  <label>Location</label>
                                   <v-select :options="renters"  :reduce="item => item.id" label="item_data" placeholder="Select renter" v-model="form.box_id" name="box_id" :on-search="loadRenters" :value="form.box_id" v-on:input="clickSearchResult"  :class="{ 'is-invalid': form.errors.has('box_id') }"></v-select>
     ...
     components: {
      ...
      methods:{
          ...
          editItem(item){
              ...
              this.form.fill(item);
              axios.post("api/monthcoveredlist?id="+this.form.box_id).then(({ data }) => (this.monthlist = data));
          },
          editPayment(){
               this.form
