`<template>
    <div class="container-fluid sales-product-item">
        ...
        <section  v-if="$gate.pos_is_NOTnull()">
             <div class="row">
                 <div class="col-sm-6">
                      <h3 style="color:#34495e">ADD DIRECT SALES - Lyrra</h3>
                    </div>
                <div class="col-md-6">
                      <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><router-link to="/dashboard">Home</router-link></li>
                        <li class="breadcrumb-item active">Add direct sales</li>
                      </ol>
                </div>
            </div>
            <div class="row justify-content-center">
                       ...
                       <div class="row ">
                        <div class="col-md-12 pb-2">
                           <div class="form-inline" autocomplete="off" style="align-items: unset;">
                                  ...
                                  <input class="form-control flex-fill" id="myInput" type="text" ref="searchProduct" placeholder="Search.." v-on:keyup.enter="searchProducts" v-model="searchProductsForm.product" v-shortkey.focus="['ctrl', 'f']" autocomplete="off"/>
                                    <ul style="padding-top: 40px;" class="list-group" id="myList" v-show="listProduct" v-shortkey="['ctrl','`']" @shortkey="fillSearchField">
                        ...
                        <div class="col-md-12 mt-2">
                         ...
                         <div class="col-md-12 mt-3">
                            <div class="form-inline clearfix">
                              <label style="padding-right: 10px;">Address: </label>
                                <input class="form-control flex-fill" id="address" type="text" ref="address"  placeholder="Address" v-shortkey.focus="['ctrl', 'n']" v-on:keyup="productPayment" v-model="form.address" :class="{ 'is-invalid': form.errors.has('address') }">
                        ...
                        <tbody>
                          <tr v-for="(productItem, index) in form.products" :key="index" v-shortkey="{tab: ['tab'], shifttab: ['shift','tab']}" @shortkey="deleteFromTable" :id="'productTableItem'+index">
                            <template v-if="index != 0">
                              ...
                              <!-- <td>{{form.products[index].remaining - form.products[index].quantity}}</td> -->
                              <td>
                                <!-- <input type="number" class="form-control" ref="searchProduct" :id="'quantityItem'+index" placeholder="Quantity" min="1" name="" v-model="form.products[index].quantity" v-on:keyup="totalPayment(index)" v-on:change="totalPayment(index)" :class="{ 'is-invalid': form.errors.has('products.'+index+'.quantity') }"> -->
                                {{form.products[index].quantity}}
                              </td>
                              <td>
                                {{form.products[index].price | toCurrency}}
                              </td>
                              <!-- <td v-model="form.products[index].total = form.products[index].quantity * form.products[index].price">{{form.products[index].total | toCurrency}}</td> -->
    ...
    export default {
     ...
     components: {
      ...
      data() {
        ...
        methods:{
          ...
          totalPayment(id = null){
            ...
            for(let i = 0; i < this.form.products.length; i++){
                sub_total = (this.form.products[i].price * this.form.products[i].quantity) + sub_total;
            }
          ...
          cancelOrder(){
            this.form.reset();
            this.totalPayment();
            localStorage.removeItem("product_direct");
          },
          printInvoice(){
            this.form
                    .post("http://localhost/printtest/api/printposdirect")
                        .then((data) => {
                     ...
                     if(quantity >= 0){
                        if(product.id){
                          ...
                          if(!order_id){
                            ...
                            // Swal.fire("Info!", "Product already added!", "info");
                          ...
                          localStorage.setItem('product_direct', JSON.stringify(this.form.products));
                        }else{
        ...
        console.log("test");
         this.form.products = eval(localStorage.getItem('product_direct')) || this.form.products;
